...
int main(int argc, char *argv[])
{
	...
	for (int k = 0; k < N_IFACE_ON_BOARD; k++)
	{
		...
		output[2] = totlen / 0xff;
		output[3] = totlen % 0xff;
		...
		//24-25 len
		totlen -= 20;
		output[24] = totlen / 0xff;
		output[25] = totlen % 0xff;
		//26-27 validation
	...
	while (1)
	{
		...
		if (time > last_time + 5 * 1000)
		{
			...
			for (int k = 0; k < N_IFACE_ON_BOARD; k++)
			{
				RipPacket rip;
				rip.command = 2;
				rip.numEntries = RoutingTable.size();
				for (int i = 0; i < RoutingTable.size(); i++)
				{
					rip.entries[i].addr = RoutingTable[i].addr&Mask(RoutingTable[i].len);
					rip.entries[i].mask = Mask(RoutingTable[i].len);
					rip.entries[i].metric = RoutingTable[i].metric;
					rip.entries[i].nexthop = RoutingTable[i].nexthop;
				}
				output[0] = 0x45;
				output[1] = 0x00;//TOS
				//2-3 total len
				...
				else
				{
					...
					for (int i = 0; i < rip.numEntries; i++)
					{
						...
						for (int j = 0; j < RoutingTable.size(); j++)
							...
							{
								if (Netaddr(RoutingTable[j]) == (Mask(mask)&addr))
								{
									found = true;
									if (curMetric <= RoutingTable[j].metric)
									{
										//update
					...
					for (int i = 0; i < RoutingTable.size(); i++)
					{
						printf("addr: %x , len: %u , if_index: %u , ")
					}
